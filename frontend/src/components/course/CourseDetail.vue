<template>
  <div>
    ...
    <div class="relative h-[75vh] overflow-hidden z-10" :class="{ 'rounded-3xl mx-4 mt-4': isDashboard }">
        ...
        <div class="relative w-full h-full">
          <transition name="hero-fade" mode="out-in">
            <img 
              ...
              :src="course?.thumbnail || '/api/placeholder/1920/1080'"
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <transition name="hero-content-fade" mode="out-in">
            <div :key="course?.id || 'default'" class="max-w-3xl mt-16">
              ...
              <div v-if="course" class="flex items-center space-x-6 mb-6 text-lg font-medium">
                <div class="flex items-center space-x-2">
                  <svg class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                  </svg>
                  <span class="text-white">{{ course.instructor?.name || course.instructor }}</span>
                </div>
                <div class="flex items-center space-x-2">
                  <div class="w-3 h-3 rounded-full" :class="getDifficultyColor(course.difficulty)"></div>
                  <span class="capitalize text-white">{{ course.difficulty }}</span>
                </div>
              ...
              <p class="text-lg md:text-xl leading-relaxed mb-8 text-dark-200 max-w-2xl font-extralight shadow-black drop-shadow-md">
                {{ course?.short_description || 'Vyberte kurz pre zobrazenie detailov.' }}
              </p>
              ...
              <div class="flex items-center space-x-4">
                ...
                >
                  <svg v-if="loading" class="animate-spin w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Content Section -->
    <div class="relative z-20 bg-dark-950 rounded-t-[3rem] -mt-8 border-t border-dark-800/50" :class="{ 'min-h-screen': !isDashboard }">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div class="lg:col-span-2 space-y-8">
                ...
                <div>
                  ...
                  <p class="text-dark-300 text-lg leading-relaxed">{{ course.description }}</p>
                </div>

                <!-- What You'll Learn -->
                <div v-if="course.what_you_will_learn && course.what_you_will_learn.length > 0">
                ...
                <div class="bg-dark-900 rounded-3xl p-6 border border-dark-800 sticky top-4 shadow-xl">
                  ...
                  <div class="space-y-4 mb-6">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="text-white capitalize font-medium">{{ course.difficulty_level }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                      <span class="text-dark-400">Dĺžka</span>
                      <span class="text-white font-medium">{{ course.duration }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                      <span class="text-dark-400">Cena</span>
                      <span class="text-green-400 font-bold text-xl">€{{ course.price }}</span>
                    </div>
                  </div>
                  ...
                  <div class="border-t border-dark-800 pt-6">
                    ...
                    <div class="mb-6">
                      ...
                      <div class="flex items-center space-x-3">
                        ...
                        <div>
                          <p class="text-white font-medium">{{ course.instructor?.name || course.instructor }}</p>
                          <p class="text-dark-400 text-sm">{{ course.instructor?.email || '' }}</p>
                        </div>
                      </div>
                    </div>
